...
When Voicebox makes requests to Stardog servers, it uses server-side connections that may require different network routing than browser-based requests. To support architectures where the Voicebox service container cannot access Stardog on public endpoints, Launchpad v3.5.0+ allows you to configure an additional internal endpoint for SSO connections.
